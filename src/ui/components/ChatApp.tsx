...
export function ChatApp({
  ...
  // ── Chunk buffer ─────────────────────────────────────────────────────────
  // Aggressively flush chunks to prevent blocking/stalling during streaming
  ...
  const appendChunk = useCallback(
    (delta: string) => {
      if (!delta) return; // Skip empty chunks

      chunkBuf.current += delta;
      // Clear existing timer to reset the interval
      if (chunkTimer.current) {
        clearTimeout(chunkTimer.current);
        chunkTimer.current = null;
      }

      // Flush immediately if buffer large, or schedule flush
      if (chunkBuf.current.length > 2_000) {
        flushChunks();
      } else {
        // Schedule flush (very aggressive: 80ms)
  ...
  const finalizeStream = useCallback(() => {
    ...
    // Flush ALL remaining chunks in the buffer
    const remaining = chunkBuf.current;
    chunkBuf.current = "";
    ...
    setSessions((prev) => {
      ...
      if (remaining && streamIdx >= 0) {
        // Add remaining chunks and mark as complete
        const updated = [...msgs];
        updated[streamIdx] = {
          ...updated[streamIdx],
          content: updated[streamIdx].content + remaining,
          streaming: false,
        };
        next.set(name, { ...s, messages: updated });
      } else if (streamIdx >= 0) {
        // No remaining chunks, just mark as complete
        const updated = [...msgs];
        updated[streamIdx] = { ...updated[streamIdx], streaming: false };
        next.set(name, { ...s, messages: updated });
      }
      return next;
    });
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      // Pre-initialize streaming message so chunks have somewhere to go
      pushMsg(currentName, { role: "assistant", content: "", streaming: true });

      await current.agent.stream(text, {
        onChunk: appendChunk,
        onToolStart: (name, input) => {
          // Flush and finalize current streaming message before tool call
          finalizeStream();
      ...
      // Critical: Force flush ALL remaining chunks immediately after stream completes
      // Don't wait for timers - ensure no data loss
      const remainingChunks = chunkBuf.current;
      if (remainingChunks) {
        chunkBuf.current = "";
        if (chunkTimer.current) {
          clearTimeout(chunkTimer.current);
        ...
        setSessions((prev) => {
          ...
          if (streamIdx >= 0) {
            ...
            updated[streamIdx] = {
              ...updated[streamIdx],
              content: updated[streamIdx].content + remainingChunks,
            };
            next.set(name, { ...s, messages: updated });
          }
          return next;
